...
const AddProduct = () => {
    ...
    const [form, setForm] = useState({
        ...
        category: "",
        material:"", 
        image: [], // State uses plural 'image'
    });
    const navigate = useNavigate();

    const handleChange = (e) => {
        const { name, value, files } = e.target;
        if (name === "image") { // Keep 'image' here to match input name
            setForm({ ...form, image: Array.from(files) }); // Store in 'image' array
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        // Append files with field name 'image' to match backend
        for (let i = 0; i < form.image.length; i++) {
            data.append("image", form.image[i]); // Changed to plural 'image'
        }
        ...
        <>
            ...
            <div className="max-w-xl mx-auto p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <input
                        type="file"
                        name="image" // Keep as 'image' to match handleChange
                        multiple
                        onChange={handleChange}
                        className="w-full"
                        required
